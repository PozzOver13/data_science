...
load(file = "progR/02R_logistic_model_coach_score.rdata")
load(file = "progR/02R_logistic_model_team_score.rdata")
load(file = "progR/02R_logistic_model_marchmadness_score.rdata")
...
#### SAVE #### 
my_sub_stage1 = my_sub_final %>%
  select(1,99)
names(my_sub_stage1) <- c("ID", "Pred")

write.table(my_sub_stage1, 
            file = "datasets/07R_submission_stage1_nowoe.csv", 
            sep = ",",
            row.names = F)
