...
## CENTROCAMPISTI
df_2016_C = df_2016_tr_cl %>%
  filter(role == "C")

df_2017_C = df_2017_tr_cl %>%
  filter(role == "C")

names(df_2016_C)

df_2016_C_nrm <- df_2016_C %>%
...
# normalizzazione
mean <- apply(df_2016_C_nrm, 2, mean)
std <- apply(df_2016_C_nrm, 2, sd)
train_data_1 <- scale(df_2016_C_nrm, center = mean, scale = std) %>% as.data.frame()
test_data_1 <- scale(df_2017_C_nrm, center = mean, scale = std) %>% as.data.frame()
...
# correlazione
corr_C <- cor(train_data_1)

corrplot(corr_C, method = "square", type = "upper")

y <- train_C$status
# XGBoost Classifier Training
xgb <- xgboost(data = data.matrix(train_C[,-1]),
               label = y,
               eta = 0.02,
               max_depth = 3, 
               nround=2000, 
...
xgb.importance(train_C[,-1] %>% names(),
               xgb) %>%
...
frm_lgt_C <- paste0("status~", stri_flatten(var_xgb, collapse = "+"))
glm_C <- glm(as.formula(frm_lgt_C), data = train_C, family = binomial)

summary(glm_C)

xgb_tr_y_pred_prob <- predict(xgb, data.matrix(train_C[,-1]))
glm_tr_y_pred_prob <- predict(glm_C, train_C, type = "response")

tr_y_pred_prob <- 0.2*xgb_tr_y_pred_prob + 0.8*glm_tr_y_pred_prob

data.frame(nm = df_2016_C$playername,
           sq = df_2016_C$squadra_classe,
...
tr_y_pred <- as.numeric(tr_y_pred_prob > 0.2)

real_1 <- train_C$status
...
df_2016_C$playername[as.numeric(rownames(train_C[train_C$status == 1,]))]
df_2016_C$playername[tr_y_pred == 1]

#### TEST ####
xgb_pred_test <- predict(xgb, data.matrix(test_C[,-1]))
glm_pred_test <- predict(glm_C, test_C, type = "response")

pred_test <- 0.2*xgb_pred_test + 0.8*glm_pred_test

pred_2 <- ifelse(pred_test > 0.2, 1, 0)
real_2 <- test_C$status
table(real_2, pred_2)
...
df_2017_C$playername[as.numeric(rownames(test_C[test_C$status == 1,]))]
df_2017_C$playername[pred_2 == 1]

min(df_2017_C$pg_tot[pred_2 == 1])
mean(df_2017_C$pg_tot[pred_2 == 1])
max(df_2017_C$pg_tot[pred_2 == 1])


df_top_C <- df_2017_C[pred_2 == 1,]

#### SAVE ####
xgb_C <- xgb
glm_C <- glm_C
cutoff_C <- 0.2
mean_var_list_C <- list(mean, std)

output_C <- list(xgb_C, glm_C, cutoff_C, mean_var_list_C)
save(output_C, file = "Google Drive/Tempo_Libero/AFSM/Output_R/04R_output_centrocampisti.rdata")
